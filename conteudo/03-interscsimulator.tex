...
O funcionamento dessa gestão de eventos baseia-se na leitura de um arquivo de configuração estático, chamado \verbatim{events.xml}, que lista as intervenções a serem realizadas. O simulador utiliza esse arquivo para agendar as modificações nos ciclos de tempo definidos. O sistema permite simular o fechamento total de vias, removendo temporariamente a aresta correspondente do grafo (close_street) e impedindo a passagem de veículos. Além disso, é possível simular a redução da capacidade de uma via (reduce_capacity), ajustando o fluxo permitido para uma porcentagem menor, o que reduz a velocidade dos carros naquele trecho.

Apesar de o gerenciador de eventos ter criado a base necessária para alterar o mapa da cidade, ele dependia originalmente de um agendamento manual. Para simular um bloqueio, era preciso saber com antecedência e definir no arquivo \verbatim{events.xml} exatamente qual rua fecharia e quando. Essa abordagem funcionava bem para situações controladas, mas não permitia relacionar diretamente o acúmulo de chuva com a interdição das ruas. Este trabalho resolve essa limitação automatizando a criação desses eventos de bloqueio a partir do processamento de dados reais de chuva, como veremos a seguir.
...
ele le o rain.csv e guarda em uma lista. 
le o roads-rain-capacity.csv e guarda em outra lista. 
processa os dois criando uma lista de eventos de alagamento. ele faz isso pegando a precipitacao acumulada de 3 horas, depois identifica quais ruas sao interditadas por essa quantidade, por fim monta o evento de close_street com duracao de 2 horas para os vértices daquela rua agrupando eles pelo momento da interdicao.como podemos ter eventos de alagamento que se sobrepoem (exemplo uma chuva forte constante que mantem a rua alagada por mais de duas horas [explicar melhor esse exemplo]) foi preciso alterar o events manager para lidar com eventos close_events quando uma rua ja esta fechada, adiando a sua abertura.
adiciona os eventos de alagamentos aos eventos lidos de events.csv

como para um fechamento completo de rua, a aresta correspondente é temporariamente removida do grafo da cidade, ao ser removida, essa via deixa de ser considerada como um caminho possível nas rotas calculadas pelos agentes, impedindo assim a passagem de qualquer carro pelo trecho interditado. quando isso acontece o carro recalcula a sua rota para o menor caminho até o seu destino a partir do vertice que ele esta usando as arestas disponiveis no grafo ou seja as ruas que nao estao interditadas.
